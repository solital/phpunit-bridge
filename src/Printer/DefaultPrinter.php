...
use Solital\PHPUnit\TestResult;
use Throwable;
...
final class DefaultPrinter
{
    ...
    {
        if (!is_null($value)) self::$compact = $value;
    ...
    {
        if (!is_null($value)) self::$profile = $value;
        return self::$profile;
    }
    ...
    {
        ConsoleOutput::line($printedUnexpectedOutput->output())->print()->break();
    }
    ...
    {
        $throwable = ThrowableBuilder::from(new \Exception($event->message()));
    ...
    public function testRunnerExecutionFinished(ExecutionFinished $event): void
    {
        ...
        if (ResultReflection::numberOfTests(Facade::result()) === 0) {
            ...
            ConsoleOutput::line("No tests found")->print()->break();
            return;
        }

        $this->style->writeCurrentTestCaseSummary($this->state);
        if (self::$compact) ConsoleOutput::line("")->print()->break();
